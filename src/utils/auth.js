...
const BASE_URL = process.env.REACT_APP_URL_PREFIX
...
const loginUser = (username, password) => {
    ...
    return tokenRequest.post(`/v1/api/token/both/`, loginBody)
        .then((response) => {
...
const refreshToken = () => {
    ...
    return tokenRequest.post(`/v1/api/token/access/`, refreshBody)
        .then((response) => {
            window.localStorage.setItem(ACCESS_TOKEN, response.data.access);
            return Promise.resolve(response.data);
        }).catch((error) => {
...
const errorInterceptor = (error) => {
    ...
    console.log("intercept not correct refresh")
    return Promise.reject(error)
}
